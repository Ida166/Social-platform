...
// === Current user identity (server-authoritative) ===//
/*Returns the current user's role and club ownership directly from the server session.
  This is the safe alternative to reading role from sessionStorage on the client. */
app.get("/me", async (req, res) => {
    const user = req.session.user;
    if (!user) {
        return res.json({ loggedIn: false, role: null, myClubId: null });
    }

    let myClubId = null;
    if (user.role === "club_owner") {
        // Look up which club belongs to this user via the users table / clubs table
    ...
    res.json({ loggedIn: true, role: user.role, myClubId });
});
...
app.delete("/events/:id/joined", async (req, res) => {
    ...
    if (error) {
        return res.status(500).json({ message: "Failed to unjoin event" });
    }
...
app.delete("/clubs/:id/joined", async (req, res) => {
    ...
    if (error) {
        return res.status(500).json({ message: "Failed to unjoin club" });
    }
...
/* Check if the current user has joined a club */
