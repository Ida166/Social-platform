import { getClubs, getEvents, getJoinCount, joinClub, unjoinClub, hasJoinedClub, getEventJoinCount, joinEvent, unjoinEvent, hasJoinedEvent, getMe } from "./clubServices.js";
...
// Role is fetched from the server — not from sessionStorage which can be tampered with.
const { role, myClubId } = await getMe();
...
async function init() {
        ...
        ? clubEvents.map(event => `
            <div class="event-card" data-event-id="${event.id}" ${club.color ? `style="border-left:4px solid ${club.color};"` : ""}>
                ...
                <div class="event-actions">
                    ${isOwner ? `<button class="button edit-event-button blue-btn">Edit</button>` : `<button class="button join-event-button" data-event-id="${event.id}">Join event</button>`}
                </div>
            </div>
        `).join("")
    ...
    // Join club button (students only)
    const joinBtn = document.getElementById("join-btn");
    if (joinBtn) {
        const [alreadyJoined, freshCount] = await Promise.all([
            hasJoinedClub(clubId),
            getJoinCount(clubId)
        ]);

        let isClubJoined = alreadyJoined;
        let memberCount = freshCount.joined;

        if (isClubJoined) {
            joinBtn.textContent = `Leave club (${memberCount} members)`;
            joinBtn.classList.add("is-joined");
        } else {
            joinBtn.textContent = `Join us (${memberCount} members)`;
        }
    ...
    // Join event buttons (students only on club page)
    if (!isOwner) {
        const eventJoinButtons = document.querySelectorAll(".join-event-button");
        eventJoinButtons.forEach(async (button) => {
            ...
            if (isJoined) {
                button.textContent = `Undo join (${countData.joined} joined)`;
            ...
            button.addEventListener("click", async () => {
                if (isJoined) {
                    ...
                    if (!result || result.error) return;
                    isJoined = false;
                    button.textContent = `Join event (${result.joined} joined)`;
                    button.classList.remove("is-joined");
                } else {
                    const result = await joinEvent(eventId);
                    if (!result || result.error) return;
                    isJoined = true;
                    button.textContent = `Undo join (${result.joined} joined)`;
                    button.classList.add("is-joined");
                }
            });
        });
