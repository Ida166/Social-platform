import { getClubs, getEvents, getJoinCount, getEventJoinCount, joinClub, joinEvent, unjoinClub, unjoinEvent, hasJoinedClub, hasJoinedEvent, getUserRole } from "./clubServices.js";
...
async function init() {
        ...
        ? clubEvents.map(event => `
            <div class="event-card" data-event-id="${event.id}" ${club.color ? `style="border-left:4px solid ${club.color};"` : ""}>
                ...
                <div class="event-actions">
                    <button class="button join-event-button hidden" data-event-id="${event.id}">Join event</button>
        ...
        <div class="content-area">
            ...
            <div class="white-box">
                ...
                <div class="info-section">
                    ...
                    <button class="join-btn hidden" id="join-btn">${alreadyJoined ? `You are joined (${members.joined}) Undo` : `Join club (${members.joined} members)`}</button>
                </div>
    ...
    // Join / unjoin button — kun synlig for studerende
    const role = await getUserRole();
    if (role === "student") {
        const joinBtn = document.getElementById("join-btn");
        joinBtn.classList.remove("hidden");
        if (alreadyJoined) joinBtn.classList.add("joined");

        function renderClubJoinButton(joined, hasJoined) {
            if (hasJoined) {
                joinBtn.textContent = `You are joined (${joined}) Undo`;
                joinBtn.classList.add("joined");
            } else {
                joinBtn.textContent = `Join club (${joined} members)`;
    ...
    // Join event knapper for studenter på club page
    if (role === "student") {
        const eventJoinButtons = document.querySelectorAll(".join-event-button");
        for (const button of eventJoinButtons) {
            const eventId = button.dataset.eventId;
            const [countData, alreadyJoinedEvent] = await Promise.all([
                getEventJoinCount(eventId),
                hasJoinedEvent(eventId)
            ]);

            button.classList.remove("hidden");

            function makeRenderEventBtn(btn) {
                return function renderEventBtn(joined, hasJoined) {
                    if (hasJoined) {
                        btn.textContent = `You are joined (${joined}) Undo`;
                        btn.classList.add("joined");
                    } else {
                        btn.textContent = `Join event (${joined} joined)`;
                        btn.classList.remove("joined");
                    }
                };
            }

            const renderEventBtn = makeRenderEventBtn(button);
            renderEventBtn(countData.joined, alreadyJoinedEvent);
            ...
            button.addEventListener("click", async () => {
                ...
                if (isJoined) {
                    ...
                    if (!result || result.error) return;
                    renderEventBtn(result.joined, false);
                } else {
                    const result = await joinEvent(eventId);
                    if (!result || result.error) return;
                    renderEventBtn(result.joined, true);
                }
            });
        }
    }
