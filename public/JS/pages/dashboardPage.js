    ...
    /* Full eventlist*/
        ...
        container.innerHTML = filteredAndSorted.map(event => `
            <div class="event-card" data-event-id="${event.id}"
                ...
                <div class="event-actions">
                    <button class="button join-event-button hidden">Join event</button>
        ...
        const joinButtons = container.querySelectorAll(".join-event-button");
            joinButtons.forEach(async (button) => {
                ...
                if (role === "student") {
                    // Student: Fortryd-knap med join/unjoin flow
                    const alreadyJoined = await hasJoinedEvent(eventId);
                    button.classList.remove("hidden");

                    function renderEventBtn(joined, hasJoined) {
                        if (hasJoined) {
                            button.textContent = `You are joined (${joined}) Undo`;
                            button.classList.add("joined");
                        } else {
                            button.textContent = `Join event (${joined} joined)`;
                            button.classList.remove("joined");
                        }
                    }

                    renderEventBtn(countData.joined, alreadyJoined);
                ...
                // Club owner: ingen join-knap vises
            });
